...
Novatree is a two-tier AI agent system that lives inside your git repository:
...
Everything is local. `.novatree/` is gitignored and each developer runs their own instance.
...
│   │   └── init.ts            # .novatree/ scaffold creation
│   ├── issues/
...
| `novatree issue create` | Create an issue from CLI |
| `novatree issue list` | List all issues |
| `novatree status` | Print summary stats |
| `novatree config set <key> <val>` | Set config with dot-notation |
| `novatree reset` | Clear all novatree state and re-init |
